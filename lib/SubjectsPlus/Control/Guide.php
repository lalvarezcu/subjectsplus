...
class Guide
{
    ...
    public function processBooks(){
        ...
        /**
         ******************************* Listing Books Pluslets ****************************************
         */

        if( isset($_POST['insert_ISBN']) && $_POST['pluslet_id'] != " " ) {

            $db = new Querier;

            //Updating since Pluslet is already on the database
            $query = "UPDATE pluslet
          SET body = " . $db->quote(scrubData($_POST['list_ISBN'], "richtext")) . "
          WHERE pluslet_id = " . scrubData($_POST['pluslet_id'], "integer");
        ...
        if( isset($_POST['insert_record']) ){
            ...
            $our_title = $_POST['book_title'];
            $our_alternate_title = $_POST['book_ISBN'];
            $our_description = $_POST['book_decription'];
            $our_prefix = 'book';

            $db  = new Querier;
                $query = "INSERT INTO title (title, alternate_title, description, pre) VALUES (
            " . $db->quote(scrubData($our_title, "text")) . ",
            " . $db->quote(scrubData($our_alternate_title, "text")) . ",
            " . $db->quote(scrubData($our_description, "richtext")) . ",
            " . $db->quote(scrubData($our_prefix, "text")) . "
            )";
            $rUpSD = $db->exec($query);
            ...
            if (!$rUpSD) {
                ...
                $this->modifyLocation($db->last_id(), 'ins');
            }

//        echo "<script type='text/javascript'>alert(". $_POST['book_ISBN'] .");</script>";
...
//$this->insertRecord();
        ...
        if(isset($_POST['edit_record'])){
            ...
            $query = "UPDATE title
                SET
                    title = " . $db->quote(scrubData("$our_title" , "text")) . ",
                    alternate_title = " . $db->quote(scrubData("$our_alternate_title", "text")) . ",
                    description = " . $db->quote(scrubData("$our_description", "richtext")) . "
                WHERE
                    title_id = " . $our_id . "
            ...
            $rUpSD = $db->query($query);

            if ($rUpSD < 0) {
                echo blunDer("We have a problem with the subject_discipline query: $query");
            }
            else{
                $this->modifyLocation($our_id, 'edt');
            }
        ...
        if(isset($_POST['delete_record'])){
            ...
            if($_POST['select_Id'] != -1){
                ...
                $query = "
                  DELETE FROM title, location_title, location
                  USING title INNER JOIN location_title INNER JOIN location
                  WHERE title.title_id = " . $our_id . "
                  AND title.title_id = location_title.title_id
                  AND location_title.location_id = location.location_id
                  ";
    ...
    function modifyLocation($book_id, $act) {

        $our_thumb = (isset($_POST['book_Thumbnail']))? $_POST['book_Thumbnail']: $_POST['edit_Thumbnail'];
        $our_tags = implode('|',$_POST["ctags"]);
        ...
        switch($act){
            case 'ins':
                ...
                $query = "INSERT INTO location ( location, access_restrictions, eres_display, display_note, ctags) VALUES (
            " . $db->quote(scrubData($our_thumb, "text")) . ",
            '" . 1 . "',
            '" . "Y" . "',
            " . $db->quote("Default Note") . ",
            " . $db->quote(scrubData($our_tags, "text")) . "
            )";

                $rInsertLoc = $db->exec($query);

                if (!$rInsertLoc) {
                    echo blunDer("We have a problem with the insert locations query: $query");
                }

                $current_location_id = $db->last_id();

                $query = "INSERT INTO location_title (title_id, location_id) VALUES (
            " . $db->quote(scrubData($book_id, "integer")) . " ,
            " . $db->quote(scrubData($current_location_id, "integer")) . "
            )";

                $query = $db->exec($query);

                if (!$query) {
                    echo blunDer("We have a problem with the insert location_title query: $query");
                }
...
//
            $query = "UPDATE location l, location_title lt
                SET
                    l.location = " . $db->quote("$our_thumb")  . ",
                    l.ctags = " . $db->quote(scrubData("$our_tags" , "text")) . "
                WHERE lt.title_id = " . scrubData($book_id, "integer") ."
                AND l.location_id = lt.location_id
                ";
//
            $rInsertLoc = $db->query($query);
